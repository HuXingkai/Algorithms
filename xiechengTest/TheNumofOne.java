...
public class TheNumofOne {
     ...
     * 链接：https://www.nowcoder.com/questionTerminal/8ee967e43c2c4ec193b040ea7fbb10b8
     ...
     !!!用相同的方法，可以判断一个数是否是2的n次幂
     (n - 1) & n==0，则此数为2的n次幂：
     原理：2的n次幂而至今中只有最高位1个1，如果我们把这个整数减1，仅有的1会变成0，其他的0将变成1，因此与操作位0
     * @param n
     * @return
     */
    public static int findOneNum(long n) {
        int count = 0;
        while (n != 0) {
            ++count;
            n = (n - 1) & n;
    ...
    public static void main(String[] args) {
        ...
        while (n!=0) {
            ...
            //直接右移方法对负数失效，将负数右移1位，由于是有符号负整数，所以最高位补1，因此会出现死循环
            //n=n>>1;
            //Java中存在无符号右移操作：“>>>”
            n = n >>> 1;
        }
